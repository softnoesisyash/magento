...
// @codingStandardsIgnoreFile change  
 ...
 */
class CarrierFedex extends \Magento\Fedex\Model\Carrier
{
    /**
     * Code of the carrier
     *
    ...
    public function collectRates(RateRequest $request)
    {
		...
		if(isset($data['addressId'])){
			...
			$addressTypeIndicator = $address->getCustomAttribute('address_type_indicator')->getValue();
            $otherAddress = $address->getCustomAttribute('other_address')->getValue(); 
		} else {
			$customAttributes = $data['address']['custom_attributes'];
			if(is_array($customAttributes)){
				foreach($customAttributes as $customAttribute){
					if ($customAttribute['attribute_code'] == 'address_type_indicator'){
						$addressTypeIndicator = $customAttribute['value'];
					}
				}
			} 
            else if($customAttribute['attribute_code'] == 'other_address'){
                $otherAddress = $customAttribute['value'];
            }   
            else {
				$addressTypeIndicator = $data['address']['custom_attributes']['address_type_indicator'];
                $otherAddress = $data['address']['custom_attributes']['other_address'];
			}
					   
		}
		
		if(strtolower($addressTypeIndicator) == strtolower('Residential')){
			$this->residentialIndicator = true; 
		} else {
			$this->residentialIndicator = false;        
		}
            ...
            'RequestedShipment' => [
                ...
                'Recipient' => [
                    'Address' => [
                        ...
                        'Residential' => $this->residentialIndicator,
                    ],
                ],
